...
### V-002 CN compliance deploy profile (public repo, no third codebase)
- **What**
  - Keep a **single public codebase** and support two deployment profiles:
    - `GLOBAL` (current public behavior)
    - `CN_COMPLIANT` (domestic-compliant profile)
- **Why**
  - Avoid maintaining a third fork while still meeting domestic distribution requirements.
- **Rule (CN_COMPLIANT)**
  - Intimacy entry must not be visible on Home.
  - Intimacy page must be non-discoverable from primary nav/entry points.
  - Any explicit/sexual intensity options must be disabled or removed from exposed flows.
  - Public-facing copy must avoid sexualized guidance language.
  - Release must pass `RELEASE_CHECKLIST_CN.md` before deploy.
- **Implementation note**
  - Prefer build/runtime flags and config toggles over branch/folder duplication.
- **Owner**
  - Product owner: AA
  - Implement/maintain: Corveil
- **Revisit**
  - Revisit when policy scope or channel strategy changes.
